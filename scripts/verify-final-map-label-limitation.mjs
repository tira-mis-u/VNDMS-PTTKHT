...
const output = path.join(root, "docs/05-architecture/final-acceptance-evidence/map-label-limitation");
...
// Deterministic audit style: no remote tile dependency. Reference five-degree graticules
...
await browser.close();
const pointLabelsOnly = geometryAudit.geometryTypes.length === 1 && geometryAudit.geometryTypes[0] === "Point";
const declaredPointBoundsMatch = JSON.stringify(geometryAudit.sourceBoundsFromMapLibreLngLatBounds) === JSON.stringify([[111.601944, 8.641667], [111.931944, 16.533333]]);
...
const summary = {
  ...
  failures: failures.length + (pointLabelsOnly && declaredPointBoundsMatch ? 0 : 1),
  authoritativeGisGeometryUsed: false,
  geometryAccuracyClaimed: false,
  limitation: "Dự án chưa có geometry GIS Hoàng Sa/Trường Sa có thẩm quyền được phê duyệt để tích hợp. UI chỉ đặt hai nhãn địa danh tại điểm neo có nguồn; không biểu diễn hoặc xác nhận polygon, đường biên, extent hay phạm vi chủ quyền.",
  provenance,
  geometryAudit,
  pointLabelsOnly,
  declaredPointBoundsMatch,
  results,
};
fs.writeFileSync(path.join(output, "map-label-limitation-verification.json"), JSON.stringify(summary, null, 2));
