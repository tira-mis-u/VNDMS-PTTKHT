...
const output = path.resolve("docs/05-architecture/ui-system-full-repair-evidence");
...
const routes = [
  ...
  ["so-tan", "/evacuations"],
  ["sos", "/sos"],
  ["diem-so-tan", "/shelters"],
  ["kho-vat-tu", "/relief/warehouses"],
  ["phan-phoi-cuu-tro", "/relief"],
  ["danh-gia-thiet-hai", "/recovery"],
  ["phan-tich", "/analytics"],
  ["bao-cao", "/analytics/reports"],
  ["mo-phong", "/simulation"],
...
const forbiddenVisible = /\b(mutation|canonical|Incident|Playbook|workspace|tick|engine|ownership|entity|snapshot|payload|query|route|drawer|modal|state|status|loading|overview|detail|source|realtime|fallback|Assessment|execution|backend|production|adapter|milestone|rollback|occupancy|utilization|capacity|revision|workflow|override)\b/i;
...
for (const [mode, viewport, theme] of modes) {
  ...
  for (const [slug, route] of routes) {
    ...
    const metrics = await page.evaluate((forbiddenSource) => {
      ...
      const sectionHeader = document.querySelector(".page-section-header");
      const sectionHeaderProblem = !sectionHeader || sectionHeader.scrollWidth > sectionHeader.clientWidth + 2;
      ...
      const measuredControls = [...new Set(controlElements)].map((element) => {
        ...
        const rect = element.getBoundingClientRect();
        return {
          kind: element.matches('button[role="combobox"]') ? "select" : element.matches(".ui-search") ? "search" : element.matches(".input-with-icon,.ai-composer > div") ? "control-wrapper" : element.tagName.toLowerCase(),
